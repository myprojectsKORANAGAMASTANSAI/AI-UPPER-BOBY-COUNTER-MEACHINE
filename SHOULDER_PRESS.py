def shoulder_press():
    ...
    def leftangle(la, lb, lc):
        ...
        cv2.rectangle(frame, (400, 25), (1000, 80), (0, 225, 225), cv2.FILLED)
        cv2.putText(frame, " SHOLUDER PRESS", (453, 60), cv2.FONT_HERSHEY_TRIPLEX, 1.5, (0, 23, 224), 1, cv2.LINE_AA)
        cv2.putText(frame, str(int(ang)), tuple(np.multiply(b, [640, 480]).astype(int)), cv2.FONT_HERSHEY_SIMPLEX, 2,
                    (255, 0, 222), 2, cv2.LINE_AA)
        return ang
    ...
    while True:
        ...
        try:
            ...
            wrist_aa = landmarks_marks[mp_pose.PoseLandmark.RIGHT_WRIST.value].x, landmarks_marks[
                mp_pose.PoseLandmark.RIGHT_WRIST.value].y
            wrist_bb = landmarks_marks[mp_pose.PoseLandmark.RIGHT_ELBOW.value].x, landmarks_marks[
                mp_pose.PoseLandmark.RIGHT_ELBOW.value].y
            wrist_cc = landmarks_marks[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].x, landmarks_marks[
                mp_pose.PoseLandmark.RIGHT_SHOULDER.value].y
            right_angle_1 = rightangle_wrist(wrist_aa, wrist_bb, wrist_cc)

            if left_angle < 50 and right_angle < 50:
                stage = "DOWN"
            elif left_angle_1 > 150 and right_angle_1 > 150 and stage == "DOWN":
                stage = "UP"
                counter_value += 1
                print(counter_value)
            ...
            right = ((right_angle + left_angle +right_angle_1+left_angle_1 )/4)-30
