def barbell_curl():
    ...
    def leftangle(la, lb, lc):
        ...
        cv2.rectangle(frame, (530, 25), (950, 90), (0, 225, 225), cv2.FILLED)
        cv2.putText(frame, "   BRA BELL CURAL", (453, 60), cv2.FONT_HERSHEY_TRIPLEX, 1.5, (0, 23, 224), 1, cv2.LINE_AA)
        cv2.putText(frame, str(int(ang)), tuple(np.multiply(b, [640, 480]).astype(int)), cv2.FONT_HERSHEY_SIMPLEX, 2,
                    (255, 0, 222), 2, cv2.LINE_AA)
    ...
    while True:
        ...
        try:
            ...
            laa = landmarks_marks[mp_pose.PoseLandmark.LEFT_WRIST.value].x, landmarks_marks[
            ...
            left_angle = leftangle(laa, lbb, lcc)
            aa = landmarks_marks[mp_pose.PoseLandmark.RIGHT_WRIST.value].x, landmarks_marks[
            ...
            right_angle = rightangle(aa, bb, cc)

            if left_angle > 160 and right_angle > 160:
                stage = "Down"

            elif left_angle < 30 and right_angle < 30 and stage == "Down":
                stage = "Up"
                counter_value += 1
                print(counter_value)
                ...
                if counter_value == 30:
                    break
            if left_angle > 160:#check
                stage = "Down"

            elif left_angle < 30 and stage == "Down":
                stage = "Up"
                wrong_value += 1

            right = 180 - ((right_angle+left_angle)/2)
