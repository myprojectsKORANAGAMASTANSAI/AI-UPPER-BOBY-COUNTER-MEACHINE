def over_head():
    ...
    def leftangle(la, lb, lc):
        ...
        cv2.putText(frame, "     OVER HEAD", (453, 60), cv2.FONT_HERSHEY_TRIPLEX, 1.5, (0, 23, 224), 1, cv2.LINE_AA)
        cv2.putText(frame, str(int(ang)), tuple(np.multiply(b, [640, 480]).astype(int)), cv2.FONT_HERSHEY_SIMPLEX, 2,
                    (255, 0, 222), 2, cv2.LINE_AA)
        return ang
    wrong_value=0
    ...
    while True:
        ...
        try:
            ...
            wrist_laa = landmarks_marks[mp_pose.PoseLandmark.LEFT_WRIST.value].x, landmarks_marks[
                mp_pose.PoseLandmark.LEFT_WRIST.value].y
            wrist_lbb = landmarks_marks[mp_pose.PoseLandmark.LEFT_ELBOW.value].x, landmarks_marks[
                mp_pose.PoseLandmark.LEFT_ELBOW.value].y
            wrist_lcc = landmarks_marks[mp_pose.PoseLandmark.LEFT_SHOULDER.value].x, landmarks_marks[
                mp_pose.PoseLandmark.LEFT_SHOULDER.value].y
            left_angle_1 = leftangle_wrist(wrist_laa, wrist_lbb, wrist_lcc)
            wrist_a = landmarks_marks[mp_pose.PoseLandmark.RIGHT_WRIST.value].x, landmarks_marks[
                mp_pose.PoseLandmark.RIGHT_WRIST.value].y
            wrist_b = landmarks_marks[mp_pose.PoseLandmark.RIGHT_ELBOW.value].x, landmarks_marks[
                mp_pose.PoseLandmark.RIGHT_ELBOW.value].y
            wrist_c = landmarks_marks[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].x, landmarks_marks[
                mp_pose.PoseLandmark.RIGHT_SHOULDER.value].y
            right_angle_1 = rightangle_wrist(wrist_a, wrist_b, wrist_c)

            if left_angle > 150 and right_angle > 150 and left_angle_1 > 150 and right_angle_1 > 150:
                stage = "UP"
            elif left_angle_1 < 70 and right_angle_1 < 70 and stage == "UP":
                stage = "DOWN"
                counter_value += 1
                print(counter_value)
            ...
            right = ((right_angle + left_angle + right_angle_1 + left_angle_1) // 4)-75
            right1 = np.interp(right, [0, 100], [400, 200])
            cv2.rectangle(frame, (570, 200), (620, 400), (0, 255, 0), 5)
            cv2.rectangle(frame, (570, int(right1)), (620, 400), (0, 23, 224), -1)
            cv2.putText(frame, stage, (10, 130), cv2.FONT_HERSHEY_TRIPLEX, 1.5, (0, 23, 224), 1, cv2.LINE_AA)
        ...
        # cv2.rectangle(frame,(10,10),(50,50),(225,225,225),cv2.FILLED)
